...
describe("JsonExtractor", () => {
  ...
  describe("tryParse", () => {
    ...
    it("should yield control to the event loop during execution", async () => {
      ...
      const tryParsePromise = (async () => {
        ...
        const result = await (jsonExtractor as any).tryParse(
          rawString,
          Infinity
        );
  ...
  describe("tryParseSync", () => {
    it("should return parsed JSON objects/arrays when valid JSON is found", () => {
      const rawString = '{"key": "value"} more text [1, 2, 3]';
      const result = (jsonExtractor as any).tryParseSync(rawString, 2);
      expect(result).toEqual([{ key: "value" }, [1, 2, 3]]);
    });
  ...
  describe("extractSync", () => {
    it("should extract multiple JSON objects/arrays from a string", () => {
      const rawString = `
        some text before a JSON object: {"key": "value"} 
        and another one: {"anotherKey": 123} 
        and an array: [1, 2, 3]
      `;
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([
        { key: "value" },
        { anotherKey: 123 },
        [1, 2, 3],
    ...
    it("should respect the limit parameter", () => {
      const rawString = `
        ...
        [1, 2, 3]
      `;
      const result = jsonExtractor.extractSync(rawString, 2);
      expect(result).toEqual([{ key: "value" }, { anotherKey: 123 }]);
    });

    it("should return an empty array if no JSON objects/arrays are found", () => {
      const rawString = "just some text without JSON";
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([]);
    });

    it("should return an empty array if the raw string is empty", () => {
      const rawString = "";
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([]);
    });

    it("should handle edge cases with nested JSON", () => {
      const rawString = `
        {"key": {"nestedKey": "nestedValue"}} 
        [1, {"key": "value"}]
      `;
    ...
    it("should handle invalid JSON gracefully and continue parsing", () => {
      const rawString = '{"key": "value"} invalid text [1, 2, 3]';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key: "value" }, [1, 2, 3]]);
    });

    it("should handle a mix of valid and invalid JSON", () => {
      const rawString = '{"key": "value"} {invalid} [1, 2, 3]';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key: "value" }, [1, 2, 3]]);
    });

    it("should extract JSON objects with special characters in keys and values", () => {
      const rawString = '{"key-with-dash": "value_with_underscore"}';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ "key-with-dash": "value_with_underscore" }]);
    });

    it("should handle JSON objects with arrays as values", () => {
      const rawString = '{"key": [1, 2, 3]}';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key: [1, 2, 3] }]);
    });

    it("should handle JSON arrays with objects as elements", () => {
      const rawString = '[{"key1": "value1"}, {"key2": "value2"}]';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([[{ key1: "value1" }, { key2: "value2" }]]);
    });
    ...
    it("should handle strings with multiple valid JSON objects separated by whitespace", () => {
      const rawString = '{"key1": "value1"}   {"key2": "value2"}';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key1: "value1" }, { key2: "value2" }]);
    });

    it("should handle strings with escaped characters in JSON", () => {
      const rawString = '{"key": "value with \\"escaped quotes\\""}';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key: 'value with "escaped quotes"' }]);
    });

    it("should handle strings with JSON containing Unicode characters", () => {
      const rawString = '{"key": "value with unicode: \\u2764"}';
      const result = jsonExtractor.extractSync(rawString);
      expect(result).toEqual([{ key: "value with unicode: ❤" }]);
    });
